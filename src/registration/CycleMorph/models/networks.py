...
def define_G(input_nc, encoder_nc, decoder_nc, which_model_netG, init_type='kaiming', gpu_ids=[]):
    if which_model_netG == 'registUnet':
        return registUnetGenerator(input_nc, encoder_nc, decoder_nc, gpu_ids=gpu_ids)
    else:
...
class Dense3DSpatialTransformer(nn.Module):
    ...
    def _interpolate(self, Input, D_upmesh, H_upmesh, W_upmesh):
        ...
        # Find batch indexes
        rep = torch.ones([depth * height * width, ]).unsqueeze_(1).transpose(1, 0)
        bDHW = torch.matmul((torch.arange(0, nbatch).float() * imgDpt * imgHgt * imgWdt).unsqueeze_(1), rep).view(
            -1).int().to(df.device)
