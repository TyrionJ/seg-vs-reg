...
from .loss import NCC_vxm, Grad3d_vxm
...
class CycleMorph(BaseModel):
    ...
    def initialize(self, opt):
        ...
        nb = opt.batchSize
        size = opt.fineSize[0]
        self.input_A = self.Tensor(nb, 1, size, size).to(opt.device)
        self.input_B = self.Tensor(nb, 1, size, size).to(opt.device)

        # load/define networks
        self.netG_A = networks.define_G(opt.input_nc, opt.encoder_nc, opt.decoder_nc, opt.which_model_net,
                                        opt.init_type, self.gpu_ids).to(opt.device)
        self.netG_B = networks.define_G(opt.input_nc, opt.encoder_nc, opt.decoder_nc, opt.which_model_net,
                                        opt.init_type, self.gpu_ids).to(opt.device)
        ...
        if self.isTrain:
            ...
            self.optimizer_ = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()),
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            _, flow_A = self.netG_A(torch.cat((real_A, real_B), dim=1).to(self.input_A.device))
        self.flow_A = flow_A
