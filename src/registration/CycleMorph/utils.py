...
def sliding_predict(model, image, tile_size, n_dims, overlap=1 / 2, flip=False):
    ...
    total_predictions = torch.zeros((1, n_dims, image_size[2], image_size[3], image_size[4]))
    count_predictions = torch.zeros((image_size[2], image_size[3], image_size[4]))
    tile_counter = 0
    print(num_rows)
    for row in range(num_rows):
        for col in range(num_cols):
            for slc in range(num_slcs):
                ...
                total_predictions[:, :, x_min:x_max, y_min:y_max,
                z_min:z_max] += predictions  # .data.cpu().numpy()
...
class Dense3DSpatialTransformer(nn.Module):
    ...
    def _transform(self, input1, dDepth, dHeight, dWidth):
        ...
        D_mesh = D_mesh.unsqueeze_(0).expand(batchSize, dpt, hgt, wdt).to(dDepth.device)
        H_mesh = H_mesh.unsqueeze_(0).expand(batchSize, dpt, hgt, wdt).to(dHeight.device)
        W_mesh = W_mesh.unsqueeze_(0).expand(batchSize, dpt, hgt, wdt).to(dWidth.device)
    ...
    def _interpolate(self, Input, D_upmesh, H_upmesh, W_upmesh):
        ...
        img = torch.zeros(nbatch, nch, depth + 2, height + 2, width + 2).to(Input.device)
        ...
        # Find batch indexes
        rep = torch.ones([depth * height * width, ]).unsqueeze_(1).transpose(1, 0).to(df.device)
        tt1 = (torch.arange(0, nbatch).float() * imgDpt * imgHgt * imgWdt).unsqueeze_(1).to(df.device)
        bDHW = torch.matmul(tt1, rep).view(-1).int()
...
def dice_val(y_pred, y_true, num_clus):
    y_pred = nn.functional.one_hot(y_pred, num_classes=num_clus)
    y_pred = torch.squeeze(y_pred, 1)
    y_pred = y_pred.permute(0, 4, 1, 2, 3)[:, 1:]

    y_true = nn.functional.one_hot(y_true, num_classes=num_clus)
    y_true = torch.squeeze(y_true, 1)
    y_true = y_true.permute(0, 4, 1, 2, 3)[:, 1:]

    intersection = y_pred * y_true
    intersection = intersection.sum(dim=[2, 3, 4])
    union = y_pred.sum(dim=[2, 3, 4]) + y_true.sum(dim=[2, 3, 4])
    dsc = (2. * intersection) / (union + 1e-5)
    return torch.mean(dsc, dim=0).cpu().numpy()
    ...
    for i in range(grid.shape[1]):
        for j in range(grid.shape[2]):
            for k in range(grid.shape[3]):
                jac_mij = [[xFX[i, j, k], xFY[i, j, k], xFZ[i, j, k]], [yFX[i, j, k], yFY[i, j, k], yFZ[i, j, k]],
