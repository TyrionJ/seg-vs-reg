...
def main(device):
    model_file = '/remote-home/hejj/Data/runtime/SoR_folder/RDP/experiments/Dataset002_InHouse_RDP_ncc-1_reg-1_lr-0.0001/dsc0.7409.pth'
    ...
    reg_model = utils.register_model(img_size, 'bilinear').to(device)
    seg_model = utils.register_model(img_size, 'nearest').to(device)

    dst_dir = f'/remote-home/hejj/Data/runtime/SoR_folder/RDP/results/{data_set}'
    ...
    with torch.no_grad():
        ...
        for idx, img_key in enumerate(img_keys):
            ...
            # subject to atlas
            def_img, flow = model(sub_img, atlas_img)
            ...
            def_seg = def_seg[0, 0].cpu().numpy().astype(np.float32)
            ...
            # atlas to subject
            def_img, flow = model(atlas_img, sub_img)
