...
def main(dataset):
    ...
    to_dir = f'{RDP_dir}/results/{dataset}'
    ...
    weights = [1, 1]
    lr = 0.0001
    model_folder = f'{dataset}_RDP_ncc-{weights[0]}_reg-{weights[1]}_lr-{lr}/'
    model_dir = f'{RDP_dir}/experiments/' + model_folder

    img_size = (160, 224, 160)
    affine = np.load(join(dr, 'affine.npy'))
    model = RDP(img_size, channels=16)
    model_file = model_dir + natsorted([i for i in os.listdir(model_dir) if 'latest' not in i])[-1]
    best_model = torch.load(model_file)['state_dict']
    print('Best model: {}'.format(model_file))

    model.load_state_dict(best_model)
    model.cuda()
    seg_model = utils.register_model(img_size, 'nearest')
    seg_model.cuda()
    ...
    print(f'\nThere are {len(test_files)} cases to infer ({dataset}):')
    with torch.no_grad():
        model.eval()
