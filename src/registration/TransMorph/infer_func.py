...
def main(device):
    model_file = '/remote-home/hejj/Data/runtime/SoR_folder/TransMorph/Dataset002_InHouse/experiments/Dataset002_InHouse_ncc_1_diffusion_1/dsc0.637.pth.tar'
    ...
    config.img_size = atlas_seg.shape
    model = TransMorph.TransMorph(config)
    best_model = torch.load(model_file)['state_dict']
    print('Best model: {}'.format(model_file))

    model.load_state_dict(best_model)
    model = model.to(device)
    reg_model = utils.register_model(config.img_size, 'bilinear').to(device)
    seg_model = utils.register_model(config.img_size, 'nearest').to(device)

    dst_dir = f'/remote-home/hejj/Data/runtime/SoR_folder/TransMorph/results/{data_set}'
    ...
    with torch.no_grad():
        ...
        for idx, img_key in enumerate(img_keys):
            ...
            sub_img, sub_seg, sub_funcs = create_clip(t1_file, ROI_file, [QSM_file], config.img_size)
