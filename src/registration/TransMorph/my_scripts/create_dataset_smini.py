...
index_json = load_json(r'E:\Data\Researches\MRI\UAI_Segm\buss\label-index.json')
...
for k, v in index_json.items():
    index_json[k] = [i - 5 for i in v]

to_dir = r'F:\Data\runtime\TransMorph\inhouse_smini'
...
def create_pickle(t1_file, roi_file):
    ...
    roi_data = process_label(roi_file)
    msk_data = nib.load(roi_file).get_fdata()
